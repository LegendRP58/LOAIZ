...
int main() {
    ...
    case 5: {
        ...
        printf("------------------------------Студенты------------------------------\n"); 
        for (int i = 0; i < stud_numbers; i++) {
            printf("%d.  Имя: %s Фамилия: %s Возраст: %d Факультет: %s Курс: %d  \n", i + 1, students[i].name, students[i].surname, students[i].age, students[i].facult, students[i].course);
        }

        printf("\n\nВыберите действие со списком:\n1.Сортировка\n2.Поиск"); //добавлена сортировка по категориям
        scanf("%d", &action); 
        switch (action) 
        {
